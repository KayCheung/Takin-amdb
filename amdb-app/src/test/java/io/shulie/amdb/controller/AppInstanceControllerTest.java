...
public class AppInstanceControllerTest extends BaseTest {
    ...
    public void setUp() throws Exception {
        ...
        criteria.andIn("appName", Lists.newArrayList("mock-test1", "mock-test2"));
        appMapper.deleteByExample(example);

        example = new Example(TAmdbAppInstanceDO.class);
    ...
    public void selectByBatchAppParams_emptyParam() throws Exception {
        ...
        //断言，参数校验不通过
        Assert.assertEquals(AmdbExceptionEnums.COMMON_EMPTY_PARAM.getCode(), result.getError().getCode());
    }
    ...
    public void selectByBatchAppParams_page5() throws Exception {
        ...
        //断言，判断返回数据条数是不是0条
        Assert.assertEquals("分页大小等于0-判断返回数据条数是不是100条", 100, result.getData().size());
    }
